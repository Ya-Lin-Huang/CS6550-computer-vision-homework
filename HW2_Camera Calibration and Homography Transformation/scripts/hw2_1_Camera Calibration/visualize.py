...
def draw_camera(ax, R, pos, color):
    ...
    leftTriangle = np.concatenate([[pos, leftTopCorner, leftBotCorner]], axis=0).squeeze()
    topTriangle = np.concatenate([[pos, leftTopCorner, rightTopCorner]], axis=0).squeeze()
    rightTriangle = np.concatenate([[pos, rightTopCorner, rightBotCorner]], axis=0).squeeze()
    botTriangle = np.concatenate([[pos, leftBotCorner, rightBotCorner]], axis=0).squeeze()
...
def visualize(pts, R1, T1, R2, T2):
    ...
    cameraPoseVector2 = cameraPoseVector2.squeeze()
    cameraPoseVector1 = cameraPoseVector1.squeeze()
    
    angle = math.degrees(math.acos(np.clip(np.dot(cameraPoseVector1, cameraPoseVector2.T), -1, 1)))
    ...
    plt.savefig('./results/CameraAngle.png',dpi = 300)
    plt.show()
